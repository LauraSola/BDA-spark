...
if(__name__== "__main__"):
  ...
  OI = OI.select(['aircraftregistration', 'starttime', 'subsystem'])
  OI = OI.withColumn("starttime", date_format(col("starttime"), "yyyy-MM-dd" ))
  OI = OI.filter( (OI['subsystem']=='3453'))
  OI = OI.withColumnRenamed('aircraftregistration','aircraftid')
  OI = OI.withColumnRenamed('starttime','timeid')
  OI = OI.select(['aircraftid', 'timeid'])
  OI = OI.distinct()
  OI = OI.withColumn("label", lit(1))
  ...
  final = DW.join(avg_sensors, ['aircraftid','timeid'], how = "inner")
  #final.show(10)

  #print(final.count())

  labelled_df = final.join(OI, ['aircraftid','timeid'], how = "left")

  #print(labelled_df.count())

  #labelled_df.show(30)

  final_prop = labelled_df.filter((labelled_df['label']== 1)).rdd.flatMap(lambda t: [((t['timeid'] - timedelta(days=days),t['aircraftid']),t['label']) for days in range(8)])\
                          .distinct().map(lambda t : (t[0][0], t[0][1], t[1])).toDF(['timeid', 'aircraftid', 'label_prop'])
  
  #final_prop.show(20)

  final_def = labelled_df.join(final_prop, ['aircraftid','timeid'], how = "left")   

  final_def = final_def.select(['aircraftid','timeid','flightcycles', 'flighthours', 'delayedminutes', 'avg(value)', 'label_prop', 'label' ])  

  print(final_def.count())
  print(final_def.filter(final_def['label_prop']==1).count())
                    

  #final_def.orderBy(col("timeid").desc()).show(20)
